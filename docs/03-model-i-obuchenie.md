...
Код: `model/encoder.py`, `model/heads_loss.py`, `model/metrics.py` (ROC-AUC / PR-AUC / ECE на numpy+scipy), `training/hybrid.py`, `baselines/` (MLP и GPS энкодеры на том же контракте фронтенда).

## 3.7 Стадии дообучения на реальные метки

Протокол 3.6 обучает модель на синтетических декоях и даёт `best_model.pth`. На нём задача насыщается (docs/05, 5.1), и содержательная дискриминация появляется только при дообучении на метки внешнего оракула. Все стадии ниже **ответвляются от одного и того же `best_model.pth`** — не последовательно друг за другом, — чтобы разница объяснялась разрешением метки, а не количеством пройденных этапов. Общий протокол: обучение на RFdiffusion + RFdiffusion-AA + EvoDiff, выбор эпохи по **шести отложенным мотивам из тридцати** (одни и те же для всех генераторов и всех стадий, `common.motifbench.motif_split`), ODesign-Rigid и GPDL целиком в холдауте; LR $5\times10^{-5}$, batch 32, сид 42, AdamW с weight decay $10^{-4}$, клиппинг нормы 1.0, bf16-автокаст.

> **Ревизия 14.08.2026.** До неё стадии различались режимом оптимизации: `soft` учился `Adam` без weight decay и клиппинга в fp32, остальные — `AdamW` с клиппингом под bf16, — то есть разница между стадиями объяснялась не только разрешением метки. Выровнено, все четыре стадии теперь гоняются одинаково.

> **Выбор эпохи раньше смотрел на знакомые мишени.** Скаффолды делились случайно внутри генератора, поэтому мотивы у train и val были общие: val мерял обобщение на новые скаффолды знакомого мотива, а не на новую мишень. Теперь под val отложены шесть мотивов целиком (`08_7AD5`, `17_7DGW`, `23_1MPY`, `25_2RKX`, `26_3B5V`, `30_7UWL`), и их же анализы помечают как `val-motif` при отчёте (docs/05, 5.6).

| Скрипт | Таргет | Лосс | Эпох | Выход |
|---|---|---|---|---|
| `training/soft.py` | $\log(1 + \mathrm{scRMSD})$, скаляр на структуру | MSE | 20 | `soft_model.pth` |
| `training/perresidue.py` | CA-lDDT, вектор длины $L$ | masked BCE | 20 | `perres_model.pth` |
| `training/joint.py` | оба сразу, веса 1:1 | MSE + masked BCE | 45 | `joint_model.pth` |
| `training/soft_multitask.py` | мягкая метка + aux-головы + native-якорь | MSE + CE | 3 × 300 шагов | `soft_model_mt.pth` |

**scRMSD** здесь — метрика MotifBench в её спецификации: $\min_k$ по восьми последовательностям, а не среднее (единая точка определения — `SCRMSD_AGG` в `common/motifbench.py`; обоснование — docs/05, 5.6).

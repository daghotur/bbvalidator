...
| Проблема | Решение |
|---|---|
| `FileNotFoundError: pca_components.pth` | выполнить шаг 5 пайплайна (`python -m preprocess.fit_pca`) |
| Даталоадер не находит h5 | пути в манифесте относительные — не перемещайте манифест отдельно от h5 |
| `--dirs` в `eval_generated.py` подхватывает лишние PDB | уточните `--pattern` (например `*/generated.pdb` вместо `**/*.pdb`) |
| Старый чекпоинт не грузится | ожидаемо: архитектура изменилась в пересборке 2026-08, нужен новый чекпоинт |

...
Обязательный гейт — `--max-scrmsd` в шкале ранжирующей модели: 2.0 Å — консервативный режим (точность 0.74–0.96, покрытие 0.6–10% по генераторам), 3.5 Å — режим покрытия (точность ~0.6, до 28%). Опциональные: `--max-clashes` (сырые конфликты геометрии Cβ < 3.5 Å, |i-j| ≥ 3), `--max-uncertainty` (MC-дисперсия ранжирующей модели, только при `--mc-runs > 1`). Независимо от гейтов verbose-вывод и CSV содержат все метрики: pred scRMSD, MC-дисперсию, диагностику похожести на натив, вероятность стерики, RMSD aux-головы, failure mode, сырые счётчики конфликтов и H-связей.

...
`filter_designability.py` батчево скорит каталог структур и для каждой печатает вердикт и конкретные заваленные метрики. Две модели с разделёнными ролями (раздел 5.4):

- **Ранжирование и вердикт** — `soft_model.pth` (дообучение на мягкую метку log1p(scRMSD)): детерминированный предсказанный scRMSD в Å, меньше = дизайнируемее; обогащение топ-10% 2.4–8.4× в зависимости от генератора.
- **Диагностика похожести на натив** — `soft_model_mt.pth` (мультизадачное дообучение с native-якорем): калиброванный предсказанный scRMSD, RMSD aux-головы, стерика, failure mode. Диагностические колонки в вердикте не участвуют; расхождение моделей («ранжируется, но не похоже на натив») помечается маркером — это полезный сигнал, а не ошибка.
...
```
Файл           |   L | scRMSD |      u | натив | RMSD_aux | p_стер | failure_mode | конфл | вердикт
01_1LDB_78.pdb | 125 |   2.52 | 0.0000 |  2.28 |     0.43 |  0.504 | ok(0.95)     |     0 | PASS
01_1LDB_24.pdb | 125 |   6.33 | 0.0000 |  7.12 |     0.80 |  0.508 | ok(0.96)     |     3 | FAIL: pred scRMSD 6.33 Å > 3.50 Å
```

Полный CSV со всеми метриками (pred scRMSD, MC-дисперсия, диагностика похожести, стерика, RMSD aux-головы, failure mode, конфликты, H-связи) сохраняется в `-o`.
